var BarcodeCapture = (function(document) {
    ...
    BarcodeCapture.prototype.parseTabSequence = function(seq) {
        var res = [];
        var code = [];
        var idx = 0;
    ...
    BarcodeCapture.prototype.tabSequence2barcode = function(sequence) {
        ...
        // - в числе по три или четыре разряда, первый всегда ноль
        // - число - ASCII-код одного символа из barcode
        //
        ...
        sequence = this.parseTabSequence(sequence);
        // В конце последовательности ожидаем три символа:
        // - CR (13) — возврат каретки
        // - LF (10) — перевод строки
        // - NULL (пусто)
        var controlSuffix = sequence.splice(Math.max(sequence.length - 2, 1))
    ...
    BarcodeCapture.prototype.startCapture = function() {
        ...
        document.addEventListener('keypress', this.keyUpHandler, false);
    ...
    BarcodeCapture.prototype.stopCapture = function() {
        ...
        this.captureStarted = false;
        document.removeEventListener('keypress', this.keyUpHandler, false);
        if (this.options.debug) {
